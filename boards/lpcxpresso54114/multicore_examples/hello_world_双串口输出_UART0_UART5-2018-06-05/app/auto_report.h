...
#define MAX_ACTOR_NUM				            7 /* please modify depend on your system *///说明：将通风模式下的风速和制冷制热模式下的风速分开来对比，所以从正常的个数多一个
#if MAX_ACTOR_NUM
...
#define FIRST_RETRY_INTERVAL                    10//s//调试用2s 正式用10s
#define SECOND_RETRY_INTERVAL                   100//s//调试用2s 正式用100s
...
{
	IDLE,//空闲，没有达到上报条件时，初始时
	STOP,//停止，上报被打断
	SETTING,//从SETTING后重新设置基准，到达条件后再上报
	WAITING//无效时间到达后设置为WAITING状态，此外未使用该状态。
}report_process_t;
...
{
	uint8_t state_flag;//状态标志	IDLE,//空闲 STOP,//停止 SETTING,//设置 WAITING//等待
...
typedef struct _u16_relay_data_t//16bits
{
	uint16_t *cur;
}u16_relay_data_t;
...
//extern u16_relay_data_t u16_relay_data[MAX_ACTOR_NUM];
//extern uint16_t u16_relay_data_bak[MAX_ACTOR_NUM];
